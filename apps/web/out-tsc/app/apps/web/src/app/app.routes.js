...
function requirePersistedAuth(source) {
...
const authGuard = () => requirePersistedAuth("canActivate");
const authChildGuard = () => requirePersistedAuth("canActivateChild");
const moduleEnabledGuard = (moduleCode) => {
    return async () => {
        const router = inject(Router);
        const accessToken = getStoredAccessToken();
