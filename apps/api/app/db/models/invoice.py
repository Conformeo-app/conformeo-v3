...
class Invoice(Base, IdentifiedModel, AuditModel, VersionedModel):
    ...
    status: Mapped[InvoiceStatus] = mapped_column(
        postgres_enum(InvoiceStatus, name="invoice_status"),
        nullable=False,
    ...
    paid_amount_cents: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
    paid_at: Mapped[date | None] = mapped_column(Date, nullable=True)
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)

    customer: Mapped["BillingCustomer"] = relationship(back_populates="invoices")
